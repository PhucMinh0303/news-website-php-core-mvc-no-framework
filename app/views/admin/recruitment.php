...
<main class="main">
    ...
    <?php if (empty($recruitments)): ?>
        <div class="empty-state" style="text-align: center; padding: 50px;">
            <p>No recruitment posts found (không có dữ liệu tin tức).</p>
            <button class="btn-primary" data-page="add-recruitment">Create your first job post</button>
        </div>
    <?php else: ?>
            ...
            <?php foreach ($recruitments as $job): ?>
                <div class="table-row">
                    <div class="col-title title-box">
                        <?php
                        $image_path = !empty($job['image']) && file_exists(__DIR__ . '/../../public/uploads/' . $job['image'])
                                ? 'uploads/' . $job['image']
...
<script>
    ...
    function deleteRecruitment(id, title) {
        if (confirm('Are you sure you want to delete "' + title + '"? This action cannot be undone.')) {
            // Thay đổi để POST đến controller
            var form = document.createElement('form');
            form.method = 'POST';
            form.action = '?page=delete-recruitment&id=' + id;
...
<style>
    /* CSS bổ sung cho status và deadline */
